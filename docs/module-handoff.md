...
- **当前质量边界**：`can_continue=true` 只表示已运行的检查未提出候选，不等于“真实无错”。独立审计找到了旧 clean 组漏报；在独立复验取得足够的真实真值前，不应把该字段单独接成无人复核的客户发送开关。详见[可用度与漏检风险审计](independent-reliability-audit.md)。
